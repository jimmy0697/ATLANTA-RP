...
Player_GiveOre(playerid, type, cooldown = 0)
{
    ...
    SendClientMessage(playerid, 0x2ECC71FF, "MINE: {FFFFFF}Tape N pour déposer le minerai extrait.");
    return 1;
}
...
Minining_PlayerStateChange(playerid, PLAYER_STATE:newstate, PLAYER_STATE:oldstate) {
    if(newstate == PLAYER_STATE_DRIVER) {
        ...
        if(Vehicle_LoadedOres(vehicleid) > 0) {
            ...
            SendClientMessage(playerid, 0x2ECC71FF, "MINE: {FFFFFF}Tu peux décharger ton minerai à l'icône du camion indiquée.");
            SendClientMessage(playerid, 0x2ECC71FF, "MINE: {FFFFFF}Utilise /mineveh pour plus d'informations.");
            for(new i; i < sizeof(DropPoints); i++) {
...
Mining_PlayerKeyStateChange(playerid, KEY:newkeys) {
    if((newkeys & KEY_NO) && CarryingOre[playerid] != -1) {
        ...
        if(id != -1) {
            if(Player_GetClosestOre(playerid, 1.5) != -1) return SendClientMessage(playerid, 0xE74C3CFF, "ERREUR: {FFFFFF}Tu ne peux pas déposer ton minerai ici.");
...
Mining_PlayerEnterDynamicCP(playerid, checkpointid) {
    if(checkpointid == LoadingPoint[playerid]) {
        ...
        if(Vehicle_LoadedOres(vehicleid) >= MINING_VEHICLE_LIMIT) return SendClientMessage(playerid, 0xE74C3CFF, "ERREUR: {FFFFFF}La benne de ton véhicule est pleine.");
...
Mining_DialogResponse(playerid, dialogid, response, listitem)
{
    switch(dialogid)
    {
        case DIALOG_ORE_TAKE:
        {
            ...
            if(MiningVein[playerid] != -1) return SendClientMessage(playerid, 0xE74C3CFF, "ERREUR: {FFFFFF}Tu es déjà en train d'extraire un minerai.");
            ...
            if(!IsValidVehicle(id)) return SendClientMessage(playerid, 0xE74C3CFF, "ERREUR: {FFFFFF}Aucun véhicule à proximité.");
            new Float: x, Float: y, Float: z;
            ...
            if(LoadedOres[id][listitem] < 1) return SendClientMessage(playerid, 0xE74C3CFF, "ERREUR: {FFFFFF}Ce véhicule ne transporte pas ce minerai.");
    ...
    if(MiningVein[playerid] != -1)
    {
        ...
        if(value >= MINING_BAR_MAX) {
            ...
            if(random(100) < OreData[ VeinData[vein_id][Type] ][Rate]) {
                ...
                GameTextForPlayer(playerid, "~n~~r~~h~Échec", 2000, 3);
            }
        }else{
