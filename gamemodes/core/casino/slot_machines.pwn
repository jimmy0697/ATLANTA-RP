	...
	if ( IsPlayerInCasino( playerid ) )
	{
		...
		if ( machineid != -1 )
		{
			...
			if ( PRESSED( KEY_JUMP ) ) {
				...
				return SendServerMessage( playerid, "Tu as %s l'auto-spin pour cette machine à sous.", p_AutoSpin{ playerid } ? ( "activé" ) : ( "désactivé" ) );
			}
			...
			if ( PRESSED( KEY_SECONDARY_ATTACK ) )
			{
				if ( g_slotmachineData[ machineid ] [ E_ROLLING ] )
					return SendError( playerid, "Attends la fin de ton tour." );
			...
			if ( PRESSED( KEY_SECONDARY_ATTACK ) )
			{
				...
				if ( id != -1 )
				{
					...
					if ( IsPlayerInRangeOfPoint( playerid, 1.0, X, Y, g_slotmachineData[ id ] [ E_Z ] ) && GetPlayerPos( playerid, Z, Z, Z ) )
					{
						...
						return ShowPlayerHelpDialog( playerid, 0, "~y~~h~~k~~PED_SPRINT~~w~ - Jouer~n~~y~~h~~k~~PED_JUMPING~~w~ - Gérer l'auto-spin~n~~y~~h~~k~~VEHICLE_ENTER_EXIT~~w~ - Quitter" );
					}
				}
			}
		}
...
public OnPlayerUseSlotMachine( playerid, slotid, first_combo, second_combo, third_combo )
{
	...
	if ( ! Iter_Contains( CasinoPool, poolid ) )
		return SendError( playerid, "Erreur au niveau de la machine à sous ! (0xFF33)" );
	...
	if ( first_combo == second_combo && first_combo == third_combo )
	{
		...
		if ( iNetWin > g_slotmachineData[ slotid ] [ E_ENTRY_FEE ] ) {
			...
			foreach ( new i : Player ) {
				SendClientMessage( i, -1, ""COL_GREY"[CASINO]{FFFFFF} %s(%d) a gagné "COL_GOLD"%s"COL_WHITE" sur une machine à sous au casino %s !", ReturnPlayerName( playerid ), playerid, cash_format( iNetWin ), g_slotmachineData[ slotid ] [ E_ENTRY_FEE ] == 10000 ? ( "Four Dragons" ) : ( g_slotmachineData[ slotid ] [ E_ENTRY_FEE ] >= 25000 ? ( "Visage" ) : ( "Caligulas" ) ) );
			}
		} else {
   			SendServerMessage( playerid, "Félicitations, tu as gagné "COL_GOLD"%s"COL_WHITE" !", cash_format( iNetWin ) );
   		}

		// give the cash
		GivePlayerMoneyEx( playerid, iNetWin );
		PlayerPlaySound( playerid, 4201, 0.0, 0.0, 0.0 ); // Coin fall
		GameTextForPlayer( playerid, "~w~~h~Gagné !", 5000, 6 );
   		return 1;
	}

	return GameTextForPlayer( playerid, "~w~~h~Pas de gain !", 2500, 6 );
}
...
stock TriggerPlayerSlotMachine( playerid, machineid )
{
	...
	if ( !g_slotmachineData[ machineid ] [ E_ROLLING ] )
	{
		...
		if ( GetPlayerMoneyEx( playerid ) < entryFee )
			return SendError( playerid, "Tu as besoin d'au moins %s pour jouer à cette machine.", cash_format( entryFee ) ), ( p_AutoSpin{ playerid } = false ), 1;
