...
{
	{"Non défini"}, //ne se produit jamais
	{"Carte Haute"},
	{"Paire"},
	{"Double Paire"},
	{"Brelan"},
...
stock ResetLabel(handle)
{
	...
	UpdateDynamic3DTextLabelTextEx(TableData[handle][E_TABLE_POT_LABEL], COLOR_GREY,
		"Appuie sur ENTRÉE pour jouer au poker\n{FFFFFF}%s Minimum\n%s / %s Blindes", cash_format(buy_in), cash_format(small_blind), cash_format(small_blind * 2));
	return 1;
}
...
stock AddPlayerToTable(playerid, handle)
{
	...
	if(slot == ITER_NONE)
	{
		SendPokerMessage(playerid, "Il n'y a actuellement aucune place libre à cette table. Tu ne peux pas la rejoindre.");
		return 0;
	}

	if(GetPlayerMoneyEx(playerid) < TableData[handle][E_TABLE_BUY_IN]) return SendPokerMessage(playerid, "Tu n'as pas assez d'argent pour accéder à cette table. Mise d'entrée : %s", cash_format(TableData[handle][E_TABLE_BUY_IN]));

	new index = Player_GetUnusedAttachIndex(playerid);
	if(index == cellmin)
	{
		SendPokerMessage(playerid, "Tu ne peux pas accéder à cette table pour le moment.");
		return 0;
	}
	//Reset player data
	memcpy(PlayerData[playerid], PlayerData[MAX_PLAYERS], 0, sizeof(PlayerData[]) * 4, sizeof(PlayerData[]));
	...
	new tstr[64];
	format(tstr, sizeof(tstr), "%s\n* en attente de la prochaine partie *", ReturnPlayerName(playerid));
	...
	SendPokerMessage(playerid, "Tu as été débité de %s pour rejoindre la table.", cash_format(TableData[handle][E_TABLE_BUY_IN]));
	...
	{
		SendPokerMessage(playerid, "Tu as rejoint cette table de poker, mais la partie a déjà commencé.");
		SendPokerMessage(playerid, "Tu dois attendre la fin de cette manche pour jouer !");
		SendTableMessage(handle, "{25728B}- - Le joueur %s a rejoint la table... - -", ReturnPlayerName(playerid));
	}
...
stock UpdateInfoTextdrawsForPlayer(playerid)
{
	new handle = PlayerData[playerid][E_PLAYER_CURRENT_HANDLE];
	new tstring[64];
	format(tstring, sizeof(tstring), "~g~Jetons :_~w~%s", cash_format(PlayerData[playerid][E_PLAYER_TOTAL_CHIPS]));
	...
	format(tstring, sizeof(tstring), "~r~Dernière_mise :_~w~%s", cash_format(TableData[handle][E_TABLE_LAST_BET]));
	PlayerTextDrawSetString(playerid, PlayerData[playerid][E_PLAYER_INFO_TXT][4], tstring);
	format(tstring, sizeof(tstring), "~r~Ta_mise :_~w~%s", cash_format(PlayerData[playerid][E_PLAYER_CURRENT_BET]));
...
public Poker_StartGame(handle, dealer)
{
	...
	for(new i = 0; i < MAX_PLAYERS; i++)
	{
		...
		{
			SendPokerMessage(i, "Tu n'as plus de jetons.");
			SendPokerMessage(i, "Tu peux rejoindre à nouveau la table et payer la mise d'entrée pour rejouer !");
			SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Le joueur %s a été exclu de la table. [Raison : plus de jetons]", ReturnPlayerName(i));
			KickPlayerFromTable(i);
	...
	if(Iter_Count(IT_PlayersTable<handle>) < 2)
	{
		SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Il n'y a pas assez de joueurs pour démarrer une partie");
		TableData[handle][E_TABLE_LOADING_GAME] = false;
		return 0;
	}
	for (new i = 0; i < 5; i ++) {
	...
	else if(count == 2)
	{
		...
		SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Le joueur %s a été désigné donneur et grosse blinde pour cette première manche !", ReturnPlayerName(dealer));
		...
		SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Le joueur %s a été désigné petite blinde pour cette première manche !", ReturnPlayerName(next_turn));
	...
	else
	{
		...
		SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Le joueur %s a été désigné donneur pour cette première manche !", ReturnPlayerName(dealer));
		TableData[handle][E_TABLE_PLAYER_DEALER_ID] = dealer;
		TableData[handle][E_TABLE_DEALER_SEAT] = PlayerData[dealer][E_PLAYER_CURRENT_CHAIR_SLOT];
		...
		SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Le joueur %s a été désigné petite blinde pour cette première manche !", ReturnPlayerName(next_player));
		TableData[handle][E_TABLE_PLAYER_SMALL_BLIND_ID] = next_player;

		//Small blind
		next_player = GetTurnAfterPlayer(handle, next_player);
		SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Le joueur %s a été désigné grosse blinde pour cette première manche !", ReturnPlayerName(next_player));
	...
	SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Le donneur mélange le jeu de cartes. Les cartes seront distribuées dans deux secondes...");
...
stock StartNewPokerGame(handle, time)
{
	...
	if(Iter_Count(IT_PlayersTable<handle>) >= 2)
	{
		SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Nouvelle partie dans %d secondes...", time);
		SetTimerEx("Poker_StartGame", 1000 * time, false, "ii", handle, INVALID_PLAYER_ID);
	}
	else
	{
		SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Il n'y a pas assez de joueurs pour démarrer une nouvelle partie !");
	}
	return 1;
}
...
public Poker_DealCards(handle)
{
	...
	if(b_big_blind && b_small_blind)
	{
		...
		SendTableMessage(handle, "{2DD9A9} * * %s mise la petite blinde de %s.. * *", ReturnPlayerName(TableData[handle][E_TABLE_PLAYER_SMALL_BLIND_ID]), cash_format(TableData[handle][E_TABLE_SMALL_BLIND]));
		SendTableMessage(handle, "{2DD9A9}  * * %s mise la grosse blinde de %s.. * *", ReturnPlayerName(TableData[handle][E_TABLE_PLAYER_BIG_BLIND_ID]), cash_format(TableData[handle][E_TABLE_BIG_BLIND]));
...
static stock UpdateTable(handle)
{
	foreach(new playerid: IT_PlayersInGame<handle>)
	{
		...
		new str[128	];
		format(str, sizeof(str), "{34c5db}Jetons : {db8d34}%s\n{db3a34}Dernière mise : {db8d34}%s", cash_format(PlayerData[playerid][E_PLAYER_TOTAL_CHIPS]), cash_format(PlayerData[playerid][E_PLAYER_CURRENT_BET]));
...
public CheckRounds(handle, bool: start_showdown)
{
	...
	switch(TableData[handle][E_TABLE_CURRENT_ROUND])
	{
		case ROUND_PRE_FLOP:
		{
			...
			SendTableMessage(handle, "{D07035}									  Le Flop 											");
			SendTableMessage(handle, "{D07035}======================================================================================");
		...
		case ROUND_FLOP:
		{
			...
			SendTableMessage(handle, "{D07035}======================================================================================");
			SendTableMessage(handle, "{D07035}									  Le Turn 											");
			SendTableMessage(handle, "{D07035}======================================================================================");
		...
		case ROUND_TURN:
		{
			...
			SendTableMessage(handle, "{D07035}======================================================================================");
			SendTableMessage(handle, "{D07035}									  La River 										");
			SendTableMessage(handle, "{D07035}======================================================================================");
...
stock CheckShowdown(handle)
{
	...
	SendTableMessage(handle, "{F25B13}									  Abattage 											");
	SendTableMessage(handle, "{F25B13}======================================================================================");
	...
	foreach(new pot_id: IT_Sidepots[handle])
	{
		...
		if(count == 1)
		{
			...
			SendTableMessage(handle, "{9FCF30}Le joueur {FF8000}%s {9FCF30}a gagné avec un(e) {377CC8}%s", ReturnPlayerName(high_id), HAND_RANKS[highest_rank >> 12]);
			SendTableMessage(handle, "{9FCF30}Gain : {377CC8}%s | -%0.0f%s pourcent de frais.", cash_format(w_chips), T_POT_FEE_RATE * 100.0, "%%");
			SendTableMessage(handle, "{9FCF30}****************************************************************************************");
		...
		{
			SendTableMessage(handle, "{9FCF30}****************************************************************************************");
			SendTableMessage(handle, "{9FCF30}Égalité ! Ces joueurs ont gagné avec un(e) {377CC8}%s :", HAND_RANKS[highest_rank >> 12]);
			new w_chips = floatround(float(TableData[handle][E_TABLE_POT_CHIPS][pot_id]) * (1.0 - T_POT_FEE_RATE));
			new fee_earned = floatround((float(TableData[handle][E_TABLE_POT_CHIPS][pot_id]) / 1000.0) * T_POT_FEE_RATE);
			UpdateServerVariable( "poker_fees", 0, GetGVarFloat( "poker_fees" ) + fee_earned, "", GLOBAL_VARTYPE_FLOAT );
          	StockMarket_UpdateEarnings( E_STOCK_CASINO, fee_earned, 0.25 );
			...
			SendTableMessage(handle, "{9FCF30}Chacun reçoit 1/%d du pot total disponible. | -%0.0f%s pourcent de frais", count, T_POT_FEE_RATE * 100.0, "%%");
			SendTableMessage(handle, "{9FCF30}****************************************************************************************");
		}
		UpdateTable(handle);
	}
...
stock bool: FoldPlayer(handle, playerid)
{
	...
	SendTableMessage(handle, "{2DD9A9}  * * %s se couche.. * *", ReturnPlayerName(playerid));
	SetPlayerChatBubble(playerid, -1, 30.0, 2000, "{D6230A}** SE COUCHE ** ");
	...
	if(count == 1)
	{
		...
		SendTableMessage(handle, "{9FCF30}****************************************************************************************");
		SendTableMessage(handle, "{9FCF30}Le joueur {FF8000}%s {9FCF30}a gagné la partie !", ReturnPlayerName(winner));
	...
	if(PlayerData[playerid][E_PLAYER_IS_PLAYING])
	{
		...
		else if(playertextid ==  PlayerData[playerid][E_PLAYER_CHOICES_TXT][CALL])
		{
			...
			if(TableData[handle][E_TABLE_LAST_BET] == PlayerData[playerid][E_PLAYER_CURRENT_BET]) //check
			{
				SetPlayerClickedTxt(playerid, true);
				KillPlayerTurnTimer(playerid);
				SendTableMessage(handle, "{2DD9A9}  * * %s check.. * *", ReturnPlayerName(playerid));
				SetPlayerChatBubble(playerid, -1, 30.0, 2000, "{22B1BD}** CHECK ** ");
			}
			else //call
			{
				...
				{
					KillPlayerTurnTimer(playerid);
					SendTableMessage(handle, "{2DD9A9}  * * %s suit %s.. * *", ReturnPlayerName(playerid), cash_format(dif));
					SetPlayerChatBubble(playerid, -1, 30.0, 2000, "{22B1BD}** SUIT %s ** ", cash_format(dif));
				...
				{
					SendPokerMessage(playerid, "ERREUR : Tu ne peux pas suivre car tu n'as pas assez de jetons. Deux options : tapis ou coucher.");
					return 1;
				}
		...
		else if(playertextid ==  PlayerData[playerid][E_PLAYER_CHOICES_TXT][RAISE])
		{
			switch(PlayerData[playerid][E_PLAYER_RCHOICE])
			{
				case E_RAISE_BET:
				{
					SendPokerMessage(playerid, "Entre un montant à miser, le total de tes jetons actuels est : %d", PlayerData[playerid][E_PLAYER_TOTAL_CHIPS]);
					ShowPlayerRaiseDialog(playerid);
					HidePlayerChoices(playerid);
				}
				case E_RAISE_RAISE:
				{
					SendPokerMessage(playerid, "Entre un montant pour relancer, le total de tes jetons actuels est : %d", PlayerData[playerid][E_PLAYER_TOTAL_CHIPS]);
					HidePlayerChoices(playerid);
					ShowPlayerRaiseDialog(playerid);
				}
				case E_RAISE_ALL_IN:
...
stock ForcePlayerAllIn(playerid, handle, bool:checkpot = true)
{
	...
	SendTableMessage(handle, "{2DD9A9}  * * %s fait tapis avec %s.. * *", ReturnPlayerName(playerid), cash_format(raise));
	SetPlayerChatBubble(playerid, -1, 30.0, 2000, "{9512CD}** TAPIS avec %s ** ", cash_format(raise));
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == DIALOG_BUY_IN)
	{
		...
		if(response)
		{
			GivePlayerMoneyEx(playerid, -TableData[handle][E_TABLE_BUY_IN]);
			PlayerData[playerid][E_PLAYER_TOTAL_CHIPS] = TableData[handle][E_TABLE_BUY_IN];
			SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Le joueur %s a payé la mise d'entrée de %s jetons pour continuer à jouer.", ReturnPlayerName(playerid), cash_format(TableData[handle][E_TABLE_BUY_IN]));
		}
		else
		{
			SendTableMessage(handle, ""COL_GREY"-- "COL_WHITE"Le joueur %s a été exclu de la table. [Raison : mise d'entrée non payée]", ReturnPlayerName(playerid));
			KickPlayerFromTable(playerid);
		}
	}
	if(dialogid == DIALOG_INPUT_RAISE)
	{
		if(response)
		{
			...
			if(TableData[handle][E_TABLE_CURRENT_STATE] != STATE_BEGIN)
			{
				SendPokerMessage(playerid, "Aucune partie active pour le moment.");
				return 0;
			}
			if(!Iter_Contains(IT_PlayersInGame<handle>, playerid))
			{
			...
			{
				SendPokerMessage(playerid, "La valeur doit être numérique.");
				ShowPlayerRaiseDialog(playerid);
				return 1;
			}
			if(raise < 0)
			{
				SendPokerMessage(playerid, "La valeur doit être supérieure à 0.");
				ShowPlayerRaiseDialog(playerid);
				return 1;
			}
			else if(raise > PlayerData[playerid][E_PLAYER_TOTAL_CHIPS])
			{
				SendPokerMessage(playerid, "Tu n'as pas autant de jetons disponibles.");
				ShowPlayerRaiseDialog(playerid);
				return 1;
			}
			else if(raise <= TableData[handle][E_TABLE_LAST_BET])
			{
				SendPokerMessage(playerid, "La valeur doit être supérieure à la dernière mise : %s", cash_format(TableData[handle][E_TABLE_LAST_BET]));
				ShowPlayerRaiseDialog(playerid);
				return 1;
			}
			else if(raise == PlayerData[playerid][E_PLAYER_TOTAL_CHIPS])
			...
			else
			{
				...
				{
					SendTableMessage(handle, "{2DD9A9}  * * %s mise %s.. * *", ReturnPlayerName(playerid), cash_format(raise));
					SetPlayerChatBubble(playerid, -1, 30.0, 2000, "{31CA15}** MISE %s ** ", cash_format(raise));
				}
				else
				{
					SendTableMessage(handle, "{2DD9A9}  * * %s relance à %s.. * *", ReturnPlayerName(playerid), cash_format(raise));
					SetPlayerChatBubble(playerid, -1, 30.0, 2000, "{31CA15}** RELANCE à %s ** ", cash_format(raise));
				}
				CheckPotAndNextTurn(playerid, handle);
			}
		}
		...
		if(handle != ITER_NONE)
		{
			if(TableData[handle][E_TABLE_VIRTUAL_WORLD] == GetPlayerVirtualWorld(playerid) && TableData[handle][E_TABLE_INTERIOR] == GetPlayerInterior(playerid))
			{
				...
				else
				{
					...
					{
						SendPokerMessage(playerid, "Tu ne peux pas quitter cette table car une partie est en cours.");
						return 0;
					}
					KickPlayerFromTable(playerid);
				}
...
stock Player_CheckPokerGame(playerid, const reason[])
{
	if(GetPVarInt(playerid, "t_is_in_table"))
    {
        ...
        SendTableMessage(handle, "Le joueur %s a été exclu de la table [RAISON : %s].", ReturnPlayerName(playerid), reason);
        KickPlayerFromTable(playerid);
    }
    return 1;
}
...
CMD:ctable(playerid, params[])
{
	...
	{
		SendPokerMessage(playerid, "/ctable [nombre de places] [petite blinde] [mise d'entrée]");
		return 1;
	}
	...
	SendPokerMessage(playerid, "Tu as créé la table ID : %d | Petite blinde : %s | Grosse blinde : %s | Mise d'entrée : %s | Places : %d", table, cash_format(small_blind), cash_format(small_blind*2), cash_format(buy_in), seats);
	return 1;
}
...
CMD:dtable(playerid, params[])
{
	...
	{
		SendClientMessageFormatted(playerid, 0x3E969FFF, "ID de table de poker invalide");
		return 1;
	}
	SendPokerMessage(playerid,  "Tu as supprimé la table de poker ID : %d", ptable);
...
CMD:agame(playerid, params[])
{
	...
	if(sscanf(params, "d", ptable))
	{
		SendPokerMessage(playerid, "/agame [ID table]");
		SendPokerMessage(playerid, "Interrompt une partie en cours et exclut tous les joueurs de la table.");
		return 1;
	}
	if(!IsValidTable(ptable))
	{
	...
	if(AbortGame(ptable))
	{
		SendPokerMessage(playerid, "Tu as interrompu avec succès la partie sur la table ID : %d", ptable);
	}
	else
	{
		SendPokerMessage(playerid, "Aucune partie n'a encore commencé à cette table.");
	}
	return 1;
}
